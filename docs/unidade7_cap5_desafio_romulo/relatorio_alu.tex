...
\begin{figure}[h!]
  \centering
  \includegraphics[width=\textwidth]{fig_esquematico_add.png}
  \caption{Esquemático do somador \textit{Ripple-Carry} de 8~bits, base das operações ADD e SUB. A cadeia de carry $C_0 \to C_1 \to \cdots \to C_{\text{out}}$ determina o caminho crítico de timing.}
  \label{fig:schem_add}
\end{figure}

O somador \textit{Ripple-Carry} (\textit{RCA}) consiste em 8~\textit{Full Adders} (FA) em cascata, cada um implementando:
\[
...
\subsubsection{Unidade Lógica e de Deslocamento}

\begin{figure}[h!]
  \centering
  \includegraphics[width=\textwidth]{fig_esquematico_logicas.png}
  \caption{Esquemáticos das 6~operações lógicas e de deslocamento do ALUChip v1.0 (representação de 1~bit). As operações lógicas são de profundidade 1~nível; os deslocamentos requerem apenas reconexão de fios (\textit{zero latency}).}
  \label{fig:schem_log}
\end{figure}
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.92\textwidth]{fig_flags.png}
  \caption{Diagrama das 4~\textit{flags} de estado do ALUChip v1.0 e suas equações booleanas de geração.}
  \label{fig:flags}
\end{figure}
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.72\textwidth]{fig_floorplan_alu.png}
  \caption{\textit{Floorplan} estimado do ALUChip v1.0 em die de 120\,\textmu m\,$\times$\,120\,\textmu m. Blocos internos: somador (azul), unidade lógica (verde), deslocador (laranja), MUX 8:1 (roxo) e lógica de flags (vermelho). Pads de I/O circundam o \textit{core}.}
  \label{fig:floor}
\end{figure}

O \textit{floorplan} distribui os três caminhos paralelos (aritmético, lógico, deslocamento) horizontalmente, com o MUX de saída ao centro e a lógica de \textit{flags} à direita. O roteamento global utiliza metal-1 (trilhas horizontais) e metal-2 (trilhas verticais), com metal-3 reservado para alimentação VDD/GND.
...
\begin{enumerate}[leftmargin=2em,label=\textbf{\arabic*.}]
  ...
  \item \textbf{Esquemáticos completos:} Somador \textit{Ripple-Carry}, unidades lógica e de deslocamento documentados.
\end{enumerate}
...
\textbf{Área de core:}             & $\approx$520\,\textmu m² \\
